...
#ifdef _WIN32
    #ifdef GymNes_LIBRARY_BUILDING_DLL
...
EXP Emulator_Handle emulator_create(const char* rom_path);
...
EXP void emulator_destroy(Emulator_Handle emulator);
...
EXP NES_Pixel_C* emulator_get_screen_buffer(Emulator_Handle emulator);
...
EXP NES_Byte_C* emulator_get_memory_buffer(Emulator_Handle emulator);
...
EXP NES_Byte_C* emulator_get_controller(Emulator_Handle emulator, int port);

/// Resets the emulator to its initial state (like pressing the reset button).
/// Loads the ROM data and initializes CPU and PPU.
///
/// @param emulator Handle to the emulator instance.
EXP void emulator_reset(Emulator_Handle emulator);

/// Executes the emulator for one full video frame.
///
/// @param emulator Handle to the emulator instance.
EXP void emulator_step(Emulator_Handle emulator);
...
///
/// @param emulator Handle to the emulator instance.
EXP void emulator_backup(Emulator_Handle emulator);
...
///
/// @param emulator Handle to the emulator instance.
EXP void emulator_restore(Emulator_Handle emulator);

/// Returns the width of the NES screen in pixels.
///
/// @return The screen width.
EXP int emulator_get_width();

/// Returns the height of the NES screen in pixels.
///
/// @return The screen height.
EXP int emulator_get_height();
